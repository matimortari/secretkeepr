...
async function handleUpdateMemberRole(memberId: string, newRole: Role) {
  if (!props.organization?.id)
    return
  try {
    await useOrganizationStore().updateOrganizationMember(memberId, newRole, props.organization.id)
...
async function handleRemoveMember(memberId: string) {
  ...
  try {
    await useOrganizationStore().removeOrganizationMember(memberId, props.organization.id)
    await useUserStore().getUser()
  }
  catch (error) {
