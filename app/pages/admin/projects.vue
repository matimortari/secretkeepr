...
async function handleCreateProject(payload: CreateProjectInput) {
  try {
    ...
    if (userStore.activeOrg?.id) {
      await projectsStore.getProjects(userStore.activeOrg.id)
    }
    isDialogOpen.value = false
  }
  catch (err: any) {
    console.error("Failed to create project:", err)
    projectsStore.errors.createProject = err.message
  }
}

watch(() => userStore.activeOrg, async (newOrg) => {
  if (newOrg) {
    try {
      await projectsStore.getProjects(newOrg.id)
    }
    catch (err: any) {
      console.error("Failed to load projects for active organization:", err)
      projectsStore.errors.getProjects = err.message
    }
  }
}, { immediate: true })
