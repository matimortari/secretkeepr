...
const { secrets } = storeToRefs(projectsStore)
...
async function handleSaveSecret(secret: any) {
  ...
  try {
    if (secret.id) {
      await projectsStore.updateProjectSecret(projectId.value, secret.id, {
        description: secret.description ?? "",
        values: secret.values ?? [],
      })
    }
    else {
      await projectsStore.createProjectSecret(projectId.value, {
        key: secret.key,
        description: secret.description ?? "",
        values: secret.values ?? [],
      })
...
watch([project, () => useUserStore().activeOrg], ([proj, activeOrg]) => {
...
watch(() => projectId.value, async (id) => {
  ...
  await projectsStore.getProjectSecrets(id)
