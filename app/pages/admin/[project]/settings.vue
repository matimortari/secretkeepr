  ...
  <div v-motion :initial="{ opacity: 0 }" :enter="{ opacity: 1 }" :duration="800">
    ...
    <section class="flex flex-col">
      <div class="md:navigation-group gap-2 border-b p-4">
        ...
        <p v-if="projectsStore.errors.updateProject" class="text-warning">
          {{ projectsStore.errors.updateProject }}
        </p>
      </div>
    ...
    <section v-if="isOwner || isAdmin" class="md:navigation-group flex flex-col justify-between gap-2 border-b p-4 md:px-10" aria-label="Add New Member">
      ...
      <div class="md:navigation-group flex flex-col gap-1">
        <div class="flex flex-row items-center gap-2">
          <input v-model="newMemberId" type="email" placeholder="Email Address" class="w-48">
        ...
        <div class="navigation-group self-end">
          <p v-if="projectsStore.errors.addProjectMember" class="text-warning">
            {{ projectsStore.errors.addProjectMember }}
          </p>
    ...
    <section v-if="isOwner" class="flex flex-col">
      ...
      <nav class="md:navigation-group flex flex-col justify-between gap-2 border-b p-4 md:px-10" aria-label="Delete Project">
        ...
        <div class="navigation-group self-end">
          <p v-if="projectsStore.errors.deleteProject" class="text-warning">
            {{ projectsStore.errors.deleteProject }}
          </p>
...
const currentUserMembership = computed(() => projectMembers.value.find((m: any) => m.userId === userStore.user?.id))
...
async function handleAddMember() {
  projectsStore.errors.addProjectMember = null
  addMemberSuccess.value = null
  if (!project.value?.id || !newMemberId.value.trim()) {
    projectsStore.errors.addProjectMember = "Email address is required."
    return
  }

  try {
    await projectsStore.addProjectMember(project.value.id, {
      email: newMemberId.value.trim(),
...
async function handleUpdateMemberRole(memberId: string, newRole: Role) {
  projectsStore.errors.updateProjectMember = null
  if (!project.value?.id)
    return
...
async function handleRemoveMember(memberId: string) {
  projectsStore.errors.removeProjectMember = null
  if (!project.value?.id)
    return
...
async function handleSubmit(index: number) {
  projectsStore.errors.updateProject = null
  if (!project.value?.id)
    return
...
async function handleDeleteProject() {
  projectsStore.errors.deleteProject = null
...
watch([() => project.value, () => userStore.activeOrg], ([proj, activeOrg]) => {
