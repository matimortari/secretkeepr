<template>
  <div class="navigation-group card justify-between border-b-2 p-4">
          ...
          <Transition name="dropdown" mode="out-in">
            <ul v-if="isDropdownOpen" class="dropdown scroll-area overflow-y-auto text-sm">
              <li
                ...
                class="truncate whitespace-nowrap rounded p-2 hover:bg-muted"
                :class="org.id === orgStore.activeOrg?.id ? 'bg-muted' : ''"
                @click="setActiveOrg(org)"
              >
                <span>{{ org.name }}</span>
              </li>
